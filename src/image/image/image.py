...
import json
import configparser
from ysk_msgs.msg import Zoom,HSVValue,YUVValue,Location,OpenCvOrder
from ysk_msgs.srv import HSVInfo,SaveHSV,YUVInfo,SaveYUV,OpenCvInfo,SaveOpenCv
...
class ImageNode(Node):
    def __init__(self):
        ...
        ws_root = Path.home() / "ysk" / "src" / "image" / "config"
        ...
        if raw:
            # 轉成絕對路徑：<workspace>/ysk/src/strategy/strategy/ar/Parameter
    ...
    def _canon_location(self, raw: str) -> str:
        ...
        # 已經是檔案（.ini 等），相對就接在 strategy_root 底下
        if p.suffix:
            return str(p if p.is_absolute() else (self.strategy_root / p))

        # 去掉開頭的 "/"，只看實際段數
        parts = [seg for seg in p.parts if seg not in (os.sep,)]
        if len(parts) == 2:
            # 像 "/ar/Parameter" 或 "ar/Parameter"
            return str(self.strategy_root / parts[0] / parts[1])

        # 其他相對路徑：也拼到 strategy_root；絕對路徑：直接用
        return str(p if p.is_absolute() else (self.strategy_root / p))    
    def _resolve_strategy_root(self) -> Path:
        # 盡量用目前檔案向上回推 <workspace>/ysk/src/strategy/strategy
        for up in Path(__file__).resolve().parents:
            if up.name == "src" and (up / "strategy" / "strategy").is_dir():
                return up / "strategy" / "strategy"
        # 找不到就退回家目錄預設
        return Path.home() / "ysk" / "src" / "strategy" / "strategy"
    ...
    def _strategy_ini_path(self) -> Path:
        env = os.environ.get("YSK_STRATEGY_INI")
        if env:
            ...
            if p is None:
                # 萬一找不到就退回家目錄一個預設位置
                p = Path.home() / "ysk/src/strategy/strategy/strategy.ini"
    ...
    def _write_autoload_js_from_raw(self, raw: str) -> str:
        ...
        env = os.environ.get("YSK_STRATEGY_AUTOLOAD")
        js_path = Path(env).expanduser().resolve() if env else Path("/home/iclab/ysk/hurocup_interface/strategy_autoload.js")
        js_path.parent.mkdir(parents=True, exist_ok=True)

        tmp = js_path.with_suffix(".tmp")
        tmp.write_text(f'window._strategy_location = {json.dumps(loc)};\n', encoding="utf-8")
        tmp.replace(js_path)
        self.loc = loc
        return loc
    ...
    def location_callback(self,msg):
        raw = (msg.data or "")
        self._write_strategy_ini_raw(raw)

        # 一行搞定：抽 loc 並寫入 autoload.js
        try:
            loc = self._write_autoload_js_from_raw(raw)
        ...
        ini = self._resolve_hsv_path()
        self.path = str(ini)
        # self.get_logger().info(f"[HSV INI] now targeting: {self.path}")
        
        # 立刻用新路徑把記憶體同步（若檔案不存在，init 會建立模板）
        self.init_hsv_from_ini(active_label=self.select_color)
